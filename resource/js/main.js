...
class Main {
    ...
    fadeAnim() {
        const eles = [...document.querySelectorAll('[data-anim]')];
        console.log(eles);
        eles.forEach(el => {
            console.log(el);
    ...
    worksFlow() {
        const lists = document.querySelector('.works_lists');
        const items = [...lists.querySelectorAll('.works_item')];
        items.forEach(el => {
            ScrollTrigger.create({
                trigger: el,
                start: `top top`,
            ...
            update: function () {
                for (var i = 0; i < dots.length; i++) {
                    ...
                    for (var j = i + 1; j < dots.length; j++) {
                        this.btg = dots[j];
                        this.bx = this.dtg.x - this.btg.x;
                        this.by = this.dtg.y - this.btg.y;
                        if (
